...
CREATE PROC XACDINH_CAPBAC_CUA_TAIKHOANHOIVIEN
    @SDT CHAR(10)
AS
BEGIN
    SELECT CB.TENCAPBAC
...
CREATE PROC TAO_TAIKHOANMOI_CHO_KH
    @HOTEN NVARCHAR(50),
    @SDT CHAR(10),
    @EMAIL NVARCHAR(50),
    @CCCD CHAR(12),
...
BEGIN
    DECLARE @MATK INT
    SELECT @MATK = MAX(MATK) + 1
    FROM TAIKHOANHOIVIEN


    INSERT INTO TAIKHOANHOIVIEN
        (MATK, HOTEN, SDT, EMAIL, CCCD, GIOITINH, NGAYSINH, DIEM_LOYALTY, SOTIENDATIEU, MACAPBAC)
    VALUES
        (@MATK, @HOTEN, @SDT, @EMAIL, @CCCD, @GIOITINH, @NGAYSINH, 0, 0, 1)
END

GO
CREATE PROC TAO_HOSOTHUCUNG
    @MATK INT,
    @TEN NVARCHAR(50),
    @LOAI NVARCHAR(20),
    @GIONG NVARCHAR(20),
    @NGAYSINH DATE,
    @GIOITINH NVARCHAR(3),
    @TINHTRANGSUCKHOE NVARCHAR(5)
AS
BEGIN
    DECLARE @MATC INT
    SELECT @MATC = MAX(MATC) + 1
    FROM THUCUNG

    INSERT INTO THUCUNG
        (MATC, MATK, TEN, LOAI, GIONG, NGAYSINH, GIOITINH, TINHTRANGSUCKHOE)
    VALUES
        (@MATC, @MATK, @TEN, @LOAI, @GIONG, @NGAYSINH, @GIOITINH, @TINHTRANGSUCKHOE)
